...
# Setup field
constraints_json = open('./res/rng_world_side_constraints.json')
...
def wfc_cycle():
    ...
    for x_offset in range(-1, 2):
        for y_offset in range(-1, 2):
            ...
            print(f'Node at ({x}, {y})')
            print(f' - Current surrounding node rel offset {x_offset, y_offset}')
            print(f'   - Abs coords {surrounding_x, surrounding_y}')
            print(f'   - Node {surrounding_node}')
            print(f'   - Type {type}')
            print(f'   - times_rotated {times_rotated}')

            surrounding_node_constraints = constraints_json[type]['tags']

            print(f'   - Constraints {surrounding_node_constraints}')
...
def check_block_legality(constraints, node):
    ...
    print(f'   - Rotated Node Constraints: {node_constraints}')

    for i in range(0, 12):
        constraint_passed = constraints[i]
        node_constraint = node_constraints[i]

        print(f'   - Constraint index: {i}')
        print(f'    - Constrain passed: {constraint_passed}')
        print(f'    - Node constraint: {node_constraint}')
